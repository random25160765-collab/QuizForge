...
(function () {
  ...
  function renderNav() {
    // 主导航由 shell.js 统一渲染（全站一份），这里只告诉它「当前视图 + 点了怎么切」。
    // 刷题页的四个视图是原地切换，所以在别的页面不需要提供 onPick（那边渲染成链接）。
    QF.shell.mount({
      view: state.view,
      onPick: function (view) {
        // 点「练习」回到选题态，但不销毁进行中的会话：
        // 选题页会给一张「继续上次练习」，误点不会丢进度。
        if (view === 'practice') state.picking = true;
        if (view === 'review') state.review = null;
        go(view);
      },
    });
  }
  ...
  function boot() {
    ...
    // 离线单文件没有路由，但顶栏的链接带 hash（quiz.html#/paper）——
    // 不读一下的话，从图谱/错题本点「组卷」会落在工作台。在线模式由 router.js 负责。
    if (!QF.router && window.location.hash) {
      var view = { '#/practice': 'practice', '#/paper': 'paper', '#/review': 'review' }[window.location.hash];
      if (view) state.view = view;
    }

    // 设置面板的导入/清空会改数据，各页面的反应不一样，所以由面板广播、页面自理
    document.addEventListener('qf:data-changed', function (event) {
      if (event.detail && event.detail.kind === 'reset') go('home');
      else render();
    });

    render();

    // 老链接（quiz.html#settings）继续可用：原地弹出，不再来回跳页
    if (window.location.hash === '#settings') QF.settings.open();
  }
