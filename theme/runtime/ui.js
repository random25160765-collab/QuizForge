...
(function () {
  ...
  /* ---------------------------------------------------------------- 图标
   *
   * **只有这一张表**。改成这样之前它散在四处（这里一张、chat.js 两张——
   * `ICONS` + `ICON_FALLBACK`、菜单一张 `MENU_ICONS`），于是同一个坑栽了三次：
   * 折叠箭头的 `chevronL/R` 只在 ui.js 里、菜单的 `bookmark/flag` 只在 chat.js 里、
   * 分享的 `share` 两张都没有 —— 表现都是"按钮里一个空 `<svg>`，不悬停根本看不见"。
   * 现在加图标只写一处，`ui.iconPath(name)` 就是那个唯一入口。
   *
   * 同名异形的那几个，取舍理由（免得以后有人以为丢了一份）：
   *   * **保留 chat 那版**：`flag`（旗=回溯，与丝带=书签成组）、`note`/`pencil`
   *    （纸=批注、笔=写描述）—— 它们都是**图标按钮**，认形状全靠它们；
   *   * **保留这里这版**：`check`/`trash`/`play`/`plus`/`close` —— app.js 里
   *    "继续 / 交卷 / 标记 / 清空进度"那类**带文字的按钮**有十几处在用，
   *    换图形比换 chat 那几处显眼得多；
   *   * 其余同名的是两边本来就画得一样。
   *
   * `notes.js` 那张**没并进来**：它的条目是 `svgIcon(...)` **预包好的整段 svg**
   *（线宽 1.6，笔记页自己的一套），与"只存路径"这张不是同一个机制 ——
   * 硬并等于顺手改笔记页的观感，不在这件事的范围里。
   */
  var ICONS = {
    ...
    // 回溯：一面小旗（趁手的位置标记）
    flag: '<path d="M6.5 3.6v17"/><path d="M6.5 4.8h10.4l-2 3.9 2 3.9H6.5z"/>',
    ...
    // 那个跟 upload / download 长得几乎一样，一排按钮里分不出来（ui.js 那边同一条注释）。
    //
    // 这一条是**补的**：`iconNode` 查的是这张表，而 `share` 原先两张表都没有 ——
    // 于是顶栏那颗分享按钮里是个空 `<svg>`，24×24 的透明方块，不悬停根本看不见
    //（用户："chatui 右上角分享的按钮没有图标，鼠标不放上去看不见"）。
    share:
      '<circle cx="17.5" cy="6" r="2.6"/><circle cx="6.5" cy="12" r="2.6"/>' +
      '<circle cx="17.5" cy="18" r="2.6"/><path d="M8.8 10.7l6.4-3.4M8.8 13.3l6.4 3.4"/>',
    ...
    // 书签：一枚丝带（永久的位置标记）。与旗要一眼分得开 —— 旗是横的、丝带是竖的
    bookmark: '<path d="M7.5 3.6h9v17l-4.5-3.8-4.5 3.8z"/>',
    ...
    // 轨迹：几个路点串成的一条路（对话树上"我走过的那条线"）
    route:
      '<path d="M5 19c0-6 3.6-11 7-11s7 5 7 11"/>' +
      '<circle cx="5" cy="19" r="1.7"/><circle cx="12" cy="8" r="1.7"/><circle cx="19" cy="19" r="1.7"/>',
    tree:
    ...
    atom:
      ...
      '<ellipse cx="12" cy="12" rx="9" ry="4" transform="rotate(-45 12 12)"/>',
    // 「skill」那颗药丸：一顶学士帽（学法/口吻那一类东西，与"深度思考"的原子同一路描边）
    skill:
      '<path d="M3 9.6 12 5l9 4.6-9 4.6z"/>' +
      '<path d="M7.2 12.1v4c0 1.2 2.1 2.2 4.8 2.2s4.8-1 4.8-2.2v-4"/>',
    // 三类 skill 各自的小图标（药丸上跟在那两个字前面）：
    //   steps = 流程（一级一级往上走）、voice = 口吻（说话）、ban = 纪律（不许）
    steps:
      '<path d="M4 18h5"/><path d="M9.5 13h5"/><path d="M15 8h5"/>',
    voice:
      '<path d="M4 5.5h16v10.5H10l-4.5 3.6V16H4z"/>',
    ban:
      '<circle cx="12" cy="12" r="7.5"/><path d="M7.2 16.8 16.8 7.2"/>',
    ...
    // 删除线：一条横线穿过一个字形的两截（与"高亮"那支荧光笔分得开）
    strike:
      '<path d="M6.5 12h11"/><path d="M9 8.2c.5-1 1.4-1.6 2.9-1.6 1.6 0 2.7.7 3.1 1.6"/>' +
      '<path d="M9.2 15.9c.5.9 1.5 1.4 2.8 1.4 1.7 0 2.9-.8 3.2-1.9"/>',
    // 下划线：字形下半段（两条竖 + 一条底线）。与"删除线"要一眼分得开 ——
    // 那条是**穿过字形中间**的横线，这条在**脚下**。
    underline:
      '<path d="M8 4.5v6.2c0 1.8 1.3 3 3.1 3s3.1-1.2 3.1-3V4.5"/><path d="M6.2 18.8h11.6"/>',
    ...
    // 分家：一条主干上岔出两支（与"删除"那只垃圾桶、与"归档"那只盒子都分得开）
    branch: '<path d="M6 4.5v15"/><path d="M6 12h4.6l6.4-6"/><path d="M6 12h4.6l6.4 6"/>',
    // 分组：一只文件夹；"移到最外层"那只多一个向上的箭头
    folder:
      '<path d="M4 7.4A1.8 1.8 0 0 1 5.8 5.6h3.3l1.7 2.2h7.4A1.8 1.8 0 0 1 20 9.6v7A1.8 1.8 0 0 1 18.2 18.4H5.8A1.8 1.8 0 0 1 4 16.6z"/>',
    folderUp:
      '<path d="M4 7.4A1.8 1.8 0 0 1 5.8 5.6h3.3l1.7 2.2h7.4A1.8 1.8 0 0 1 20 9.6v7A1.8 1.8 0 0 1 18.2 18.4H5.8A1.8 1.8 0 0 1 4 16.6z"/>' +
      '<path d="M12 16.2v-4.4"/><path d="m9.9 13.9 2.1-2.1 2.1 2.1"/>',
