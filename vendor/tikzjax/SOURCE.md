...
## 三层分工与回归套件（2026-09-26）

用户："**不能一直靠正则给 llm 擦屁股，这样没完没了啊**" —— 对。根子是**回路是断的**：引擎的
报错应用听不到，模型永远不知道自己错在哪，于是只能我拿正则一条条猜。现在按三层分工来：

* **引擎层（我）**：只做"让它能说话"与**无损**的事 —— 预载库、日志桥（含把 `! …` 那句单独留住）、
  精确点名、摘掉说明性的前置命令、按需套 CJK、中文坐标键改写、按样式键补库。
  **有损的改写一律不做**：`shader=interp` 从前被我悄悄摘掉 —— 那是**改了用户的图**（少了渐变，
  而正文还写着"按高度上色"），现已撤掉，改成如实报错、交给模型重画。
  同理撤掉了"把标签里的 `!` 裹成 `{!}`"那处：它是按一张**简化版**图二分出来的，对完整那张
  并不成立。**规矩：没有用例支撑的改写一律不加。**
* **模型层（提示词）**：给它**验过的写法**（见下"两条硬规矩"），失败时把**引擎原话**回给它。
* **应用层（客户端）**：图编不出来时**自动请模型重画一次** —— `latex.js` 把本批失败交给
  `QF.latex.onFail`，`chat.js` 的 `armingRepair` 把它连同**模型原来写的源码**发回去，用户不必开口。
  一轮只重画一次（改完还不行就如实说），而且**等它说完再发**（半路插消息会打断那一轮）。

**回归套件**：`tools/texlab/cases.json`（17 条）＋ `run_cases.js` ＋ `report.py`，跑法 `make cases`。
每条用例**重开页面**、走**应用自己的渲染链**（`QF.latex.slot/fill`）—— 摘前言命令、按需套 CJK、
按样式键补库、中文键改写、日志桥、缓存全在覆盖之内。**规矩：以后每加一处兜底，必须同时带一条
用例；没有用例的改写一律不加。** 首次全绿：**17 / 17**（其中四条"应当失败"的用例，还检查文案里
是否带上了引擎原话 —— 它们正是用来钉住"报错能被听见"这件事的）。

**两条硬规矩**（tikz-cd，都是按引擎原话订的，各有一条用例）：

* 箭头只能指向**有内容**的格子：空格子没有节点，报 `No shape named tikz@f@1-3-3`；
* 位置词跟**标签**走：`\arrow[dd, dashed, "v" near start]` ✓，`\arrow[dd, "v", dashed near start]` ✗
  （报 `I do not know the key '/tikz/dashed near start'`）。
