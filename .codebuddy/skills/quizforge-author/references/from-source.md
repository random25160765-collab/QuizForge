...
1. **先写母本**：只管问序与因果，不写答案，把整条链铺完。母本要回答的是
   「这条路径上依次会遇到哪些瓶颈、每一问的动机是不是上一问的果」。
2. **验问序**：把顺序打乱。如果仍然成立，说明它只是题集，回去重排。
3. **再挑路面**：从母本里挑一段入库（挑分支点密集、且能独立成立的那段），
   补齐 `#### 参考答案` 与 `#### 评分要点`。

母本本身**不当作一道题交付**，它留作地图：入库的题可以是「一段路」，
但必须有人手里拿着整张地图，否则下一批题会重复覆盖同一段、而某些段永远没人走。
